...
export async function register(req: Request, res: Response) {
  try {
    const { name, surname, email, password, phone, universityId, courseOfStudy, tosConsent, marketingConsent } = req.body;
    if (!name || !surname || !email || !password) {
      res.status(400).json({ error: 'Nome, cognome, email e password sono obbligatori' });
      return;
    }
    const result = await registerUser({ name, surname, email, password, phone, universityId, courseOfStudy, tosConsent: !!tosConsent, marketingConsent: !!marketingConsent });
